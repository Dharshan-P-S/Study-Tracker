...
import { updateImageNotes } from '../api/imageApi';
...
const ImageNotesModal = ({ isOpen, onRequestClose, image, onUpdate }) => {
  ...
  const handleSaveChanges = async () => {
    try {
      await updateImageNotes(image._id, notes);
      onUpdate();
      onRequestClose();
    } catch (error) {
    ...
    >
      ...
      <div className="max-h-[70vh] overflow-y-auto pr-2">
        <div className="p-4 bg-slate-100 dark:bg-slate-900 rounded-lg flex flex-col gap-3 mb-4">
          ...
          <input type="text" placeholder="URL or File Path" value={newNoteUrl} onChange={(e) => setNewNoteUrl(e.target.value)} className="w-full p-2 bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-md" />
        ...
        <div className="space-y-3">
          ...
          )) : <p className="text-center text-slate-500 dark:text-slate-400 py-4">No notes added for this image yet.</p>}
        </div>
      </div>
