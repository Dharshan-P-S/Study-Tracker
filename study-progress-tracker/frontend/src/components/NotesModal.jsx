...
const NotesModal = ({ isOpen, onRequestClose, topic, onUpdate }) => {
  ...
  const handleAddNote = () => {
    ...
    if (newNote) {
      ...
      handleSaveChanges(null, updatedNotes); // Save immediately after adding Text/Link
      resetForms();
    }
  };
  ...
  const handleDeleteNote = (indexToDelete) => {
    ...
    setNotes(updatedNotes);
    handleSaveChanges(null, updatedNotes); // Save immediately after deleting
  };

  const handleSaveChanges = async (newImageFile = null, notesToSave = notes) => {
    try {
      const updatedTopic = await updateTopicNotes(topic._id, notesToSave, newImageFile);
      setNotes(updatedTopic.notes); // Update local state with saved notes (important for image uploads)
      onUpdate(); // Refresh board
      if (!newImageFile) {
        // Only close if saving text/link notes or deleting.
        // Keep open if uploading image, as it resets.
        // onRequestClose(); // Optional: Keep modal open after saving text/links?
      }
    } catch (error) {
  ...
  const filteredNotes = useMemo(() => {
    return notes.filter(note => note.noteType === activeTab);
  }, [notes, activeTab]);
  ...
  return (
    ...
    >
      ...
      <div className="max-h-[70vh] flex flex-col">
        ...
        <div className="p-4 bg-slate-100 dark:bg-slate-900 rounded-lg mb-4">
          ...
          <div className="flex flex-col gap-3">
            {activeTab === 'Text' && (
              <textarea placeholder="Enter your text note..." value={textNoteContent} onChange={(e) => setTextNoteContent(e.target.value)} className="w-full h-24 p-2 bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-md" />
            )}
            {activeTab === 'Image' && (
               <input type="file" accept="image/*" onChange={(e) => setImageNoteFile(e.target.files[0])} className="w-full text-sm text-slate-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-slate-100 dark:file:bg-slate-600 file:text-blue-700 dark:file:text-blue-300 hover:file:bg-slate-200 dark:hover:file:bg-slate-500"/>
            )}
            {activeTab === 'Link' && (
              <>
        ...
        {/* List of existing notes - NOW FILTERED */}
        <div className="space-y-3 overflow-y-auto flex-grow pr-2">
          {filteredNotes.length > 0 ? filteredNotes.map((note, index) => {
             // Find the original index in the unfiltered 'notes' array for deletion
             const originalIndex = notes.findIndex(n => n === note); 
             return (
                <div key={originalIndex} className="flex items-start justify-between p-3 bg-slate-50 dark:bg-slate-700 rounded-md">
                  ...
                  <button type="button" onClick={() => handleDeleteNote(originalIndex)} className="text-red-500 hover:text-red-700 font-bold px-2 flex-shrink-0">✕</button>
                </div>
             )
          }) : <p className="text-center text-slate-500 dark:text-slate-400 py-4">No {activeTab.toLowerCase()} notes added yet.</p>}
        </div>
      </div>
      ...
      <div className="flex justify-end gap-4 mt-8 pt-4 border-t border-slate-200 dark:border-slate-700">
        ...
        {/* Removed the Save All Notes button as saves happen immediately */}
      </div>
    </Modal>
  );
};
