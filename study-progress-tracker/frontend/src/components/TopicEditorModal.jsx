...
const TopicEditorModal = ({
  ...
  topic,
  onTopicUpdate,
  isOutdated, // ✅ passed from StudyBoardPage
}) => {
  ...
  useEffect(() => {
    if (topic) {
      const parsedDue = topic.dueDate ? new Date(topic.dueDate) : null;
      setTitle(topic.title || '');
      setDueDate(parsedDue);
      setOriginalDueDate(parsedDue); // ✅ capture original once
    } else {
      setTitle('');
      setDueDate(null);
  ...
  const dueDateChanged = () => {
    if (!originalDueDate && !dueDate) return false;
    if (!originalDueDate && dueDate) return true;
    if (originalDueDate && !dueDate) return true;
    return originalDueDate.getTime() !== dueDate.getTime();
  };
  ...
  const handleSave = async (e) => {
    ...
    // 🚫 BLOCK ONLY HERE
    if (isOutdated && dueDateChanged()) {
      alert('You cannot change the due date of an outdated topic. Recreate it instead.');
      return;
    }
  ...
  return (
    ...
    >
      ...
      <form onSubmit={handleSave}>
        <div className="mb-4">
          <label className="block mb-1 font-semibold text-slate-600 dark:text-slate-300">
            Title
          </label>
          <input
            type="text"
