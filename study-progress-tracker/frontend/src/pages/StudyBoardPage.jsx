...
import { logStudySession } from '../api/sessionApi'; // <-- Fixes the "not defined" error
import Timer from '../components/Timer';
import { DndContext, closestCenter } from '@dnd-kit/core';
import { SortableContext, useSortable, verticalListSortingStrategy } from '@dnd-kit/sortable';
import { CSS } from '@dnd-kit/utilities';

// This component should NOT be wrapped in a <Link>
const TopicCard = ({ topic }) => {
  ...
  return (
    <div ref={setNodeRef} style={style} {...attributes} {...listeners} className="p-3 bg-white border rounded-lg shadow-sm">
      <h4 className="font-semibold">{topic.title}</h4>
    </div>
  );
};

const Column = ({ id, title, topics }) => {
      ...
      <DndContext collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
        <div className="flex flex-col md:flex-row gap-4">
          {Object.entries(columns).map(([status, topicsInColumn]) => (
            <Column key={status} id={status} title={status} topics={topicsInColumn} />
          ))}
        </div>
      </DndContext>
    </div>
